...
import { List, User, LogOut, Plane } from "lucide-react";
      ...
      >
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            {isAuthenticated ? user?.username : t("header.cabinet")}
          </button>
            ...
            <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl py-3 z-50 animate-fadeIn border border-gray-100">
              {isAuthenticated ? (
                <>
                  <Link
                    to="/cabinet"
                    className="flex items-center gap-3 px-4 py-2.5 text-[#2E3034] hover:bg-blue-50 transition font-medium"
                    onClick={() => setIsDropdownOpen(false)}
                  >
                    <Plane className="w-4 h-4 text-[#4BACE1]" />
                    {t("header.myBookings")}
                  </Link>
                ...
                <>
                  <Link
                    ...
                    onClick={() => setIsDropdownOpen(false)}
                  >
                    <User className="w-4 h-4 text-[#4BACE1]" />
                    {t("cabinet.login")}
                  </Link>
                  <Link
                    to="/cabinet"
